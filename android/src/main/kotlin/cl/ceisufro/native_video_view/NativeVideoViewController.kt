...
class NativeVideoViewController(
    ...
    private fun initVideo(dataSource: String?) {
        this.configurePlayer()
				val json = URL("http://gambatvonline.com/backend/public/api/movie/get_useragent").readText()
				val jsonSinComillas = json.replace("\"", "")
				Log.d("key","json ${jsonSinComillas}")
				val header = HashMap<String, String>(1)
				header.put("User-Agent", jsonSinComillas)
        if (dataSource != null) {
            val videoUri = Uri.parse(dataSource)
            this.videoView?.setVideoURI(videoUri, header)
    ...
    private fun notifyPlayerPrepared(mediaPlayer: MediaPlayer?) {
        ...
        Log.d("NOM#getLanguage", " TrackInfo size: $tamanio")
        var trackInfo = mediaPlayer?.getTrackInfo()
        
        val nameTable = mutableMapOf<Int, String>()
        
        var i = 0
        while(i < tamanio) {
            var it = mediaPlayer?.getTrackInfo()[i]
            Log.d("NOM#notifyPlayerPrepared", "getTrackInfo ${i} ${it} ${it.getTrackType()} ${it.getLanguage()}")
            Log.d("NOM#getLanguage", "it.getTrackType() ${it.getTrackType()} ")
            if(it.getTrackType() == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_AUDIO){
                nameTable[i] = it.getLanguage()!!
            }
            i++
        }
        Log.d("NOM#getLanguage", " Track(s) Audio: ${nameTable.size} ")
